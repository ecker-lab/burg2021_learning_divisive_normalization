...
from divisivenormalization.models import ConvNet
...
def main():
    ...
    base_dir = "/projects/burg2021_learning-divisive-normalization/cnn3"
    fname = "results.csv"
    param_dict = {
        "run_no": None,
    ...
    # DN model parameters unused but written to csv.
    pool_type = "None"
    pool_kernel_size = [-1, -1, -1, -1]
    ...
    for _ in range(1000):
        ...
        smooth_weights = [log_uniform(low=-9, high=-3.5), 0, 0]
        readout_sparse_weight = log_uniform(low=-5.5, high=-1)
        output_nonlin_smooth_weight = log_uniform(low=-8, high=2)
